...
resource "aws_s3_bucket" "k4-secret-s3-bucket" {
  bucket = "k4-secret-s3-bucket-${local.bucket_suffix}"
  force_destroy = true
  tags = {
      Name = "k4-secret-s3-bucket-${local.bucket_suffix}"
      Description = "CloudGoat ${var.cgid} S3 Bucket used for storing a secret"
      Stack = "${var.stack-name}"
      Scenario = "${var.scenario-name}"
  }
}
resource "aws_s3_bucket_object" "k4-shepards-credentials" {
  bucket = "${aws_s3_bucket.k4-secret-s3-bucket.id}"
  key = "admin-user.txt"
  source = "../assets/admin-user.txt"
  tags = {
    Name = "k4-shepards-credentials-${var.cgid}"
    Stack = "${var.stack-name}"
    Scenario = "${var.scenario-name}"
  }
}
