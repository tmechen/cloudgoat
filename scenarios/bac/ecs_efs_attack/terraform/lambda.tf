...
resource "aws_iam_role_policy_attachment" "k4-lambda-policy" {
  role       = "${aws_iam_role.k4-lambda-role.name}"
  policy_arn = "arn:aws:iam::aws:policy/AWSLambdaExecute"
}

resource "aws_iam_role_policy_attachment" "k4-lambda-policy2" {
  role       = "${aws_iam_role.k4-lambda-role.name}"
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaVPCAccessExecutionRole"
}

resource "aws_iam_role_policy_attachment" "k4-lambda-policy3" {
  role       = "${aws_iam_role.k4-lambda-role.name}"
...
resource "aws_lambda_function" "efs_upload" {
  ...
  function_name = "k4-efs_upload-${var.cgid}"
  ...
  vpc_config {
    ...
    subnet_ids         = ["${aws_subnet.k4-public-subnet-1.id}"]
    security_group_ids = ["${aws_security_group.k4-ec2-efs-security-group.id}"]
  }
...
resource "aws_cloudwatch_event_rule" "k4_insert_file_every_three_minutes" {
  name                = "k4_every_three_minutes_${var.cgid}"
  description         = "Fires every_three_minutes"
  schedule_expression = "rate(3 minutes)"
}

resource "aws_cloudwatch_event_target" "k4_check_insert_file_every_three_minutes" {
  rule      = "${aws_cloudwatch_event_rule.k4_insert_file_every_three_minutes.name}"
  target_id = "lambda"
...
resource "aws_lambda_permission" "allow_cloudwatch_to_call_insert_lambda" {
  ...
  source_arn    = "${aws_cloudwatch_event_rule.k4_insert_file_every_three_minutes.arn}"
}
