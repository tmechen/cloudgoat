...
resource "aws_s3_bucket" "k4-cardholder-data-bucket" {
  bucket = "k4-cardholder-data-bucket-${local.bucket_suffix}"
  force_destroy = true
  tags = {
      Name = "k4-cardholder-data-bucket-${local.bucket_suffix}"
...
resource "aws_s3_bucket_object" "cardholder-data-primary" {
  bucket = "${aws_s3_bucket.k4-cardholder-data-bucket.id}"
